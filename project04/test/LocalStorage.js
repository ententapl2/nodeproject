...
import AuthUserQuery from "../src/app/queryDto/AuthUserQuery.js";
import PollQueryMapper from "../src/data/mapper/PollQueryMapper.js";
...
function achteTest() {
    ...
    const t = query.length > 0 ? {
        id: query[0]['user.id'],
        name: query[0]['user.name'],
        password: query[0]['user.password'],
        roles: query.map(row => ({
            id: row['role.id'],
            name: row['role.name']
        }))
...
function neuneTest() {
    ...
    FROM ( 
        ...
        FROM 
            poll 
        WHERE 
            poll.question LIKE '%' || :phrase || '%' COLLATE NOCASE 
        LIMIT :limit 
        OFFSET :offset 
    ) AS userPoll 
